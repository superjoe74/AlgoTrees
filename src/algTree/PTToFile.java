...
public class PTToFile {
	...
	private void writeChild(FileWriter f, Node from, Node to, String s) throws IOException {
		...
		if (to.m_Left != null || to.m_Right != null) {
			f.write("\n[");
			if (to.m_Left != null && (to.m_BitPos < to.m_Left.m_BitPos)) {
				if (to.m_Right != null && (to.m_BitPos < to.m_Right.m_BitPos)) {
					...
					if (to.m_BitPos < to.m_Left.m_BitPos) {
						if (to.m_Left == leaf) {
							writeChild(f, to, to.m_Left, "))");
						}else {
							writeChild(f, to, to.m_Left, "[]))");
				...
				if (to.m_Right != null && (to.m_BitPos < to.m_Right.m_BitPos)) {
					if (to.m_Right == leaf) {
						writeChild(f, to, to.m_Right, "[]))");
					}else if(to.m_BitPos < to.m_Right.m_BitPos){
						writeChild(f, to, to.m_Right, "))");
					}
					f.write("))]");
				}
